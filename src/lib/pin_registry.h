 ...
 *
 * @param showAvailableOnly If true, show only unallocated physical pins
 * @param specificPin Pin number to query (0xFF = show all pins)
 *
 * Usage:
 *   printPinStatus();           // Show all pin allocations
 *   printPinStatus(true);       // Show only available pins
 *   printPinStatus(false, A0);  // Query specific pin
 */
void printPinStatus(bool showAvailableOnly = false, uint8_t specificPin = 0xFF);
