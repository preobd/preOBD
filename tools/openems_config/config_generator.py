...
def generate_advanced_config_block(inputs: List[Dict[str, Any]], tool_version="1.0.0") -> Optional[str]:
    """
    Generates the advanced_config.h block for custom calibrations.
    Returns None if no custom calibrations are defined.
    """
    # Check if any inputs have custom calibrations
    inputs_with_calibrations = [inp for inp in inputs if 'custom_calibration' in inp]
...
"""
    ...
    for inp in inputs_with_calibrations:
        cal = inp['custom_calibration']
        input_num = inp['input_number']
        app_name = inp.get('application', 'NONE')
        cal_name = cal.get('calibration_name', 'UnknownCalibration')

        cal_block = f"""
// ----- Input {input_num}: {app_name} Custom Calibration -----
#define INPUT_{input_num}_CUSTOM_CALIBRATION
#ifdef INPUT_{input_num}_CUSTOM_CALIBRATION
"""

        if cal_name == "ThermistorSteinhartCalibration":
            cal_block += f"""    static const PROGMEM {cal_name} input_{input_num}_custom_cal = {{
        .bias_resistor = {cal['bias_resistor']},
        .steinhart_a = {cal['steinhart_a']},
        .steinhart_b = {cal['steinhart_b']},
        .steinhart_c = {cal['steinhart_c']}
    }};
"""
        elif cal_name == "LinearCalibration":
            cal_block += f"""    static const PROGMEM {cal_name} input_{input_num}_custom_cal = {{
        .voltage_min = {cal['voltage_min']},
        .voltage_max = {cal['voltage_max']},
        .pressure_min = {cal['pressure_min']},
        .pressure_max = {cal['pressure_max']}
    }};
"""
        elif cal_name == "PolynomialCalibration":
            cal_block += f"""    static const PROGMEM {cal_name} input_{input_num}_custom_cal = {{
        .bias_resistor = {cal['bias_resistor']},
        .poly_a = {cal['poly_a']},
        .poly_b = {cal['poly_b']},
...
"""
    ...
    return header + "".join(calibration_blocks)
...
def update_advanced_config_h(advanced_config_path: str, new_block: Optional[str]) -> bool:
    """
    Surgically replaces the custom calibrations block in advanced_config.h and creates a backup.
    If new_block is None, clears the custom calibrations section.
    ...
    try:
        ...
        with open(advanced_config_path + ".bak", 'w') as f:
            f.write(content)

        # Look for the CUSTOM CALIBRATIONS section marker
        start_marker = "// CUSTOM CALIBRATIONS"
        end_marker = "#endif // ADVANCED_CONFIG_H"

        start_index = content.find(start_marker)
        end_index = content.find(end_marker)

        if start_index != -1 and end_index != -1:
            # Find the start of the section (beginning of the comment block)
            ...
            pre_block = content[:line_start]
            post_block = "\n" + end_marker + content[end_index + len(end_marker):]

            if new_block:
                new_content = pre_block + new_block + "\n" + post_block
            else:
                # Clear custom calibrations but keep the file structure
                new_content = pre_block + "\n" + post_block
        else:
            # Markers not found, append before final #endif
