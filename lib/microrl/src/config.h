/*
Microrl library config files - Customized for openEMS
Original Author: Eugene Samoylov aka Helius (ghelius@gmail.com)
*/
...
#define _COMMAND_LINE_LEN (1+80)  // 80 chars + NULL terminator
...
"IRin> set mode test" */
#ifdef ARDUINO_MEGA
    #define _COMMAND_TOKEN_NMB 8   // Limited for Mega (tight RAM)
#else
    #define _COMMAND_TOKEN_NMB 16  // More tokens for Teensy/ESP32
#endif
...
#define _PROMPT_DEFAUTL "openEMS> "  // Note: typo in microrl v1.5.1 - should be _PROMPT_DEFAULT
...
#define _PROMPT_LEN 9  // Length of "openEMS> "
...
#ifdef ENABLE_TERMINAL_EMULATOR
    #define _USE_COMPLETE  // Enable tab completion (terminal mode only)
#endif

/*
Define it, if you wanna use history. It works like bash history, and
...
NOTE: History requires _USE_ESC_SEQ to be enabled for UP/DOWN arrow keys. */
#ifdef ENABLE_TERMINAL_EMULATOR
    #define _USE_HISTORY  // Enable command history (terminal mode only)
#endif
...
#ifdef ARDUINO_MEGA
    #define _RING_HISTORY_LEN 0    // No history buffer on Mega
#else
    #define _RING_HISTORY_LEN 128  // Larger history buffer for Teensy/ESP32
#endif
...
#ifdef ENABLE_TERMINAL_EMULATOR
    #define _USE_ESC_SEQ  // Enable ESC sequences (terminal mode only)
#endif
...
*/
#ifdef ARDUINO_MEGA
    #undef _USE_LIBC_STDIO  // Use lightweight version on Mega
#else
    #define _USE_LIBC_STDIO // Use standard library on Teensy/ESP32
#endif
...
#define _ENABLE_INIT_PROMPT  // Print prompt on init

/*
New line symbol
Terminal mode uses CR for compatibility with screen/minicom/picocom.
Serial monitor mode uses LF for Arduino/PlatformIO serial monitors. */
#ifdef ENABLE_TERMINAL_EMULATOR
    #define _ENDL_CR  // Use CR for terminal emulators
#else
    #define _ENDL_LF  // Use LF for basic serial monitors
#endif
